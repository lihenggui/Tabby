...
class FilesClient(private val context: Context) {
  suspend fun list(parentDocumentId: String): List<FilesClientDocument> =
  ...
  suspend fun importDocument(parentDocumentId: String, source: Uri, name: String) =
    withContext(Dispatchers.IO) {
  ...
  suspend fun copyDocument(documentId: String, source: Uri) {
    val target = buildDocumentUri(documentId)

    context.contentResolver.copyContentTo(source, target)
  }

  suspend fun copyDocument(target: Uri, documentId: String) {
